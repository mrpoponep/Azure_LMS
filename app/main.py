...
def remove_content(CourseID,ContentID):
    # Assuming you have a model function to remove content
    print(ContentID)
    ...
    # Redirect back to the course page after content removal
...
def create_quizz(CourseID):
    ...
    if request.method == "POST":
        Title = request.form.get("Title", "")
        quizz_id, success, message = model.create_quizz(CourseID, Title, TeacherID)
        # Process each question and its details
        question_count = int(request.form.get('questionCounter'))
        questions = []
        for i in range(question_count):
            question_content = request.form.get(f'questions[{i}].content')
            question_type = request.form.get(f'questions[{i}].type')
            question_options = request.form.getlist(f'questions[{i}].options[]')
            question_answer = request.form.get(f'questions[{i}].answer')

            # Create a dictionary with question details
            question_data = {
        ...
        # Now you have 'title' and 'questions' containing all the form data
        # You can perform further processing or store the data in a database
        print(questions)
        if success:
            flash("Quizz created successfully.", "success")
            return redirect(url_for("course", CourseID=CourseID))
        flash(message, "warning")
        return redirect(url_for("course", CourseID=CourseID))
